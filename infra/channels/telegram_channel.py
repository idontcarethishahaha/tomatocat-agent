...
class TelegramChannel:
    ...
    async def send_image(self, chat_id: str, image: str) -> None:
        """发送图片（本地路径或 URL）；GIF 会作为动画发送"""
        cid = int(self._resolve_chat_id(chat_id))
        is_gif = image.lower().endswith(".gif")
        if image.startswith(("http://", "https://")):
            if is_gif:
                await self._telegram_outbound_limiter.run(
                    ...
                    action=lambda: self._app.bot.send_animation(chat_id=cid, animation=image),
                )
            else:
                await self._telegram_outbound_limiter.run(
                    cid,
            ...
            if is_gif:
                await self._telegram_outbound_limiter.run(
                    ...
                    kind="send",
                    label="send_animation",
                    action=lambda: self._send_animation_file(cid, image),
                )
            else:
                await self._telegram_outbound_limiter.run(
                    cid,
    ...
    async def _send_animation_file(self, chat_id: int, file_path: str) -> object:
        with open(file_path, "rb") as f:
            return await self._app.bot.send_animation(chat_id=chat_id, animation=f)
