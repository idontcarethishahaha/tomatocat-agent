...
class AppRuntime:
    def __init__(self, config: Config, workspace: Path, config_path: str = "config.toml") -> None:
        self.config = config
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            server_port = _read_server_port(self.config, self.config_path)
...
def _read_server_port(config: Config, config_path: str = "config.toml") -> int:
    """从配置文件读取 server.port，默认 2236"""
    try:
        import tomllib
        ...
        if path.exists():
            with open(path, "rb") as f:
                data = tomllib.load(f)
            return data.get("server", {}).get("port", 2236)
    except Exception:
        pass
    return 2236


def build_app_runtime(config: Config, workspace: Path | None = None, config_path: str = "config.toml") -> AppRuntime:
    return AppRuntime(config, workspace or (Path.home() / ".akashic" / "workspace"), config_path)
