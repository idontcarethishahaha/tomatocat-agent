...
class TelegramChannel(Channel):
    ...
    async def _watch_polling(self) -> None:
        """Recover a polling loop that exited or stopped consuming updates."""
        while self._application and self._polling:
            await asyncio.sleep(_POLL_HEALTH_INTERVAL_S)
            ...
            try:
                ...
                if pending > 0 and pending == self._last_pending_updates:
                    ...
                    self._stale_pending_checks = 0
                self._last_pending_updates = pending

                if self._stale_pending_checks >= _POLL_STALE_CHECKS:
                    logger.warning(
